...
heloc_schema = detect_schema(heloc, target_column="RiskPerformance")

X_train, X_test, y_train, y_test, preprocessor = prepare_dataset(
    heloc, "RiskPerformance", heloc_schema
)
...
bank_schema = detect_schema(bank, target_column="y")

X_train, X_test, y_train, y_test, preprocessor = prepare_dataset(bank, "y", bank_schema)
