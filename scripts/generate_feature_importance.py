...
OUTPUT_DIR = Path("data/exports/shap")
...
def generate_importance_for_model(dataset_name, dataset_path, target_column):
    ...
    shap_rows = min(SHAP_SAMPLE_SIZE, X_test_processed.shape[0])

    X_shap = X_test_processed[:shap_rows]

    print(f"\nUsing {shap_rows} rows " f"for SHAP analysis")

    for model_name, model_builder in MODELS.items():

        print(f"\nGenerating SHAP for " f"{model_name}")
        ...
        shap_values = generate_shap_values(explainer, X_shap)

        importance_df = calculate_feature_importance(shap_values, feature_names)

        filename = f"{dataset_name.lower()}_" f"{model_name}_" f"importance.csv"

        output_file = OUTPUT_DIR / filename

        export_feature_importance(importance_df, output_file)

        print(f"Exported: " f"{output_file}")
...
if __name__ == "__main__":
    ...
    for dataset_name, config in DATASETS.items():
        ...
        generate_importance_for_model(dataset_name, config["path"], config["target"])
