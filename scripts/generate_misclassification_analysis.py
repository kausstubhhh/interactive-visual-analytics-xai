...
EXPORT_DIR = Path("data/exports/errors")
...
def export_error_summary(summary, output_file):
...
def run_analysis(dataset_name, dataset_path, target_column):
    ...
    for model_name, model_builder in MODELS.items():
        ...
        summary = calculate_error_summary(y_test, results["predictions"])

        output_file = EXPORT_DIR / f"{dataset_name.lower()}_" f"{model_name}_errors.csv"

        export_error_summary(summary, output_file)
...
if __name__ == "__main__":
    ...
    for dataset_name, config in DATASETS.items():
        ...
        run_analysis(dataset_name, config["path"], config["target"])
