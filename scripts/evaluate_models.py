...
from src.evaluation.exporter import export_evaluation_summary
...
def print_results(dataset_name, model_name, results):
    ...
    confusion = results["confusion_values"]
...
def evaluate_dataset(dataset_name, dataset_path, target_column, summary_results):
    """
    ...
    for model_name, model in models.items():
        ...
        prediction_results = fit_and_predict(
            model, X_train_processed, y_train, X_test_processed
        )

        evaluation_results = evaluate_model(
            y_true=y_test,
            y_pred=prediction_results["predictions"],
            y_proba=prediction_results["probabilities"][:, 1],
        )

        print_results(dataset_name, model_name, evaluation_results)
...
if __name__ == "__main__":
    ...
    for dataset_name, config in DATASETS.items():
        ...
        evaluate_dataset(
            dataset_name, config["path"], config["target"], summary_results
        )

    export_evaluation_summary(
        results=summary_results, output_path="data/exports/evaluation_summary.csv"
    )
