...
DATASET_PATH = Path("data/raw/heloc_dataset_v1.csv")
...
def main():
    ...
    shap_values = generate_shap_values(explainer, X_test_processed)

    feature_names = preprocessor.get_feature_names_out()

    importance_df = calculate_feature_importance(shap_values, feature_names)

    top_features = get_top_features(importance_df, top_n=10)
    ...
    print(f"SHAP shape: " f"{shap_values.values.shape}")

    print(f"Feature count: " f"{len(feature_names)}")
