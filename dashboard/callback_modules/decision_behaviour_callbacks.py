...
def register_decision_behaviour_callbacks(app):
    @app.callback(
        ...
        Output("decision-metric-cards", "children"),
        Output("decision-chart", "figure"),
        Output("decision-table", "data"),
        Output("decision-table", "columns"),
        Input("analysis-status", "children"),
        Input("decision-model-dropdown", "value"),
        Input("decision-topn-dropdown", "value"),
    )
    def update_decision_behaviour(
        ...
        df = load_local_explanation_file(dataset, model)
        ...
        df = df.rename(columns={"contribution": "SHAP Value"})
        summary = generate_decision_summary(df, model)

        metric_cards = build_decision_metric_cards(df, model, top_n)
        figure = local_shap_chart(df)
