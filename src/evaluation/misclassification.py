...
def get_false_positives(y_true, y_pred):
    ...
    mask = (df["actual"] == 0) & (df["predicted"] == 1)
...
def get_false_negatives(y_true, y_pred):
    ...
    mask = (df["actual"] == 1) & (df["predicted"] == 0)

    return df[mask]


def calculate_error_summary(y_true, y_pred):
    """
    ...
    false_negatives = get_false_negatives(y_true, y_pred)
