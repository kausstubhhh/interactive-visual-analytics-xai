...
def load_uploaded_dataset(file_path: str | Path) -> pd.DataFrame:
    """
...
def preview_dataset(df: pd.DataFrame, rows: int = 10) -> pd.DataFrame:
    """
...
def detect_dataset_schema(df: pd.DataFrame, target_column: str) -> dict:
    """
    Wrapper around schema detector.
    """
    return detect_schema(df=df, target_column=target_column)
